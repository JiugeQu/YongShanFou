...
function displayData(data) {
    ...
    orderStatusElement.textContent = order.state === 3 ? '备餐中' : '';
    orderStatusElement.textContent = order.state === 4 ? '备餐中' : '';
    orderStatusElement.textContent = order.state === 5 ? '已出餐' : '';
    orderStatusElement.textContent = order.state === 6 ? '已出餐' : '';
...
// 发送 PUT 请求的函数
function sendPutRequest(orderId,newState) {
    var apiUrl = `http://127.0.0.1:4523/m1/3576146-0-default/staff/order/${orderId}`;
